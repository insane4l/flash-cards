...
export const Cards = () => {
    ...
    const showPackId = useAppSelector(state => state.packs.showPackId)
    const editPackId = useAppSelector(state => state.packs.editPackId)
    ...
    useEffect(() => {
        dispatch(getCardsTC({cardsPack_id: currentPack, page, pageCount: 8}))
    ...
    const updateCardsHandler = (id: string, question: string) => {
        dispatch(updateCardTC(id, question))
    }
    ...
    const sortHandler = () => {
        if (sort === '0updated') {
            dispatch(getCardsTC({cardsPack_id: currentPack, page, pageCount: 8, sortCards: '0updated'}))
            setSort('1updated')
        } else {
            dispatch(getCardsTC({cardsPack_id: currentPack, page, pageCount: 8, sortCards: '1updated'}))
            setSort('0updated')
        }
    }
